"""
Make an LLM call to Claude to get the time.

Claude uses a tool (Python function) to get the time from an API.
Pydantic AI uses the input type TimeInput to require a timezone in string format.
The Pydantic AI output type "TimeOutput" forces Claude to return the time as a string and the UTC offset as a string.
Flow control is orchestrated by Langgraph.
After the time is fetched by the tool, another LLM call is made asking the LLM to identify what month it is and
always reply with the name of the month plus an emoji.
Then, flow control determines whether the time is AM or PM.
The result is enriched with the AM/PM info and the final output is provided to the user.
"""
import urllib3
import json
...
time_agent = Agent(model, deps_type=TimeInput, output_type=TimeOutput, system_prompt=('Use the `get_time` function to get the time.'))
month_name_agent = Agent(model, output_type=MonthNameOutput, system_prompt=('Return the name of the month and also return an emoji representing that month.'))
...
def get_time_node(state: AgentState) -> AgentState:
    time_data = time_agent.run_sync(state['get_time_prompt'], deps=TimeInput(state['timezone']))
...
def get_month_name_node(state: AgentState) -> AgentState:
    month_name_data = month_name_agent.run_sync(f"Given this datetime string, what month is it?: {state['time_data']}")
...
def format_response(state: AgentState) -> AgentState:
    state['final_answer'] = {'current_time': f"{state['time_data']}",
...
def is_AM_or_PM(state: AgentState) -> Literal["is_AM", "is_PM"]:
    ...
    if datetime.strptime(state['final_answer']['current_time'] ,'%Y-%m-%dT%H:%M:%S.%f%z').hour < 12:
        return "is_AM"
    else:
        return "is_PM"
...
def main():
    ...
    config = {
        "get_time_prompt": "What is the current time? Return your answer in ISO time format.",
        #"timezone": "Asia/Seoul"
